        ...
        if (!$message) {
            try {
                ...
                if ($editing_id) {
                    ...
                    $stmt->execute([$key ?: $editing['key'], $title, $image_path, $description, $editing_id]);
                ...
                if (isset($_POST['related-title']) && !empty($_POST['related-title'])) {
                    ...
                    foreach ($_POST['related-title'] as $index => $related_title) {
                        ...
                        $related_image_path = '';
                        $map_link = isset($_POST['map-link'][$index]) ? sanitize($_POST['map-link'][$index]) : '';

                        error_log("Related Place [$index]: Title=$related_title, Description=$related_description, Map Link=$map_link");
                        ...
                        // Insert related place
                        error_log("Inserting related place #$index");
                        $stmt = $pdo->prepare("INSERT INTO related_places (activity_id, title, image, description, map_link) VALUES (?, ?, ?, ?, ?)");
                        $stmt->execute([$editing_id, $related_title, $related_image_path, $related_description, $map_link]);
                ...
                if (!$message) {
                    ...
                    ob_end_clean();
                    header("Location: /travelguide/thingsadmin.php");
                    exit;
                }
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if (!$activity) {
            ...
            $pdo->prepare("DELETE FROM related_places WHERE activity_id = ?")->execute([$id]);
...
// Fetch all activities and their related places
try {
    $stmt = $pdo->query("SELECT * FROM activities");
    $activities = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch related places for each activity (still needed for deletion and editing)
    $related_places_by_activity = [];
    foreach ($activities as $act) {
        $stmt = $pdo->prepare("SELECT * FROM related_places WHERE activity_id = ?");
        $stmt->execute([$act['id']]);
        $related_places_by_activity[$act['id']] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (PDOException $e) {
...
<head>
  ...
  <style>
    * {
    ...
    .related-place-item .form-group {
      flex: 1;
    }

    .related-place-item label {
      color: #0288d1;
    }

    .related-place-item input[type="text"],
    .related-place-item input[type="file"] {
      width: 100%;
    }

    .related-place-item button.remove-btn {
      background: #f97316;
      color: white;
      border: none;
      padding: 0.5rem 1rem;
    ...
    .related-place-item button.remove-btn:hover {
      background: #e55e00;
    }
            ...
            <?php if ($editing && $editing['related']): foreach ($editing['related'] as $index => $place): ?>
              <div class="related-place-item">
                ...
                <div class="form-group">
                  ...
                  <input type="url" name="map-link[]" value="<?php echo htmlspecialchars($place['map_link'] ?? ''); ?>" placeholder="e.g., https://maps.google.com" />
                </div>
                <button type="button" class="remove-btn">Remove</button>
              </div>
            <?php endforeach; endif; ?>
          </div>
          <button type="button" id="add-related-place">Add Nearby Place</button>
        </div>
        <div class="form-buttons">
          <button type="submit">Save Activity</button>
          <a href="/travelguide/thingsadmin.php" class="cancel-btn">Cancel</a>
        </div>
      </form>
    </div>
        ...
        <?php else: ?>
          <?php foreach ($activities as $act): ?>
            <tr>
              ...
              <td class="actions">
                <a href="/travelguide/thingsadmin.php?edit=<?php echo $act['id']; ?>" class="edit-btn">Edit</a>
                <a href="/travelguide/thingsadmin.php?delete=<?php echo $act['id']; ?>" class="delete-btn" onclick="return confirm('Are you sure you want to delete <?php echo htmlspecialchars($act['title']); ?>?');">Delete</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
  ...
  <script>
    ...
    function addRelatedPlaceInputs() {
      ...
      relatedPlaceItem.innerHTML = `
        ...
        <div class="form-group">
          <label>Nearby Place Map Link (Optional)</label>
          <input type="url" name="map-link[]" placeholder="e.g., https://maps.google.com" />
        </div>
        <button type="button" class="remove-btn">Remove</button>
      `;
      relatedPlacesList.appendChild(relatedPlaceItem);
      relatedPlaceItem.querySelector('.remove-btn').addEventListener('click', () => {
